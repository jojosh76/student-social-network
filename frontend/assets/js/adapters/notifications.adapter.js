    ...
    _bindCardEvents() {
        document.querySelectorAll('.notif-card').forEach(card => {
            card.addEventListener('click', (e) => {
                ...
                NotificationService.markRead(card.dataset.id).catch(console.error);
        ...
        document.querySelectorAll('.mark-read').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                NotificationService.markRead(btn.dataset.id).catch(console.error);
        ...
        document.querySelectorAll('.notif-action-btn.del').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                NotificationService.delete(btn.dataset.id).catch(console.error);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('clearAll')?.addEventListener('click', () => {
        ...
        if (window.AppUI) window.AppUI.notify('Notifications effacées.', 'success');
    });
    ...
    try {
        ...
        NotificationStore.load(data.map(_normalizeNotification));
    } catch (err) {
...
function _normalizeNotification(n) {
    const createdAt = new Date(n.createdAt || Date.now());
    return {
        id: n.id,
        type: n.type || 'system',
        actor: { name: 'StudNet', initials: 'SN' },
        message: `<strong>${n.title || 'Notification'}</strong>${n.body ? ` - ${n.body}` : ''}`,
        preview: n.preview || '',
