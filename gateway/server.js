...
const SERVICES = {
    ...
    messages : process.env.MESSAGE_SERVICE_URL  || 'http://localhost:5001',
    posts    : process.env.POST_SERVICE_URL     || 'http://localhost:5002',
    users    : process.env.USER_SERVICE_URL     || 'http://localhost:5003',
    events   : process.env.EVENT_SERVICE_URL    || 'http://localhost:5004',
    search   : process.env.SEARCH_SERVICE_URL   || 'http://localhost:5005',
    files    : process.env.FILE_SERVICE_URL     || 'http://localhost:5006',
...
app.use('/api/posts',         makeProxy(SERVICES.posts));
app.use('/api/users',         makeProxy(SERVICES.users));
app.use('/api/events',        makeProxy(SERVICES.events));
app.use('/api/conversations', makeProxy(SERVICES.messages));
app.use('/api/search',        makeProxy(SERVICES.search));
app.use('/api/upload',        makeProxy(SERVICES.files, { selfHandleResponse: false }));
...
io.on('connection', (clientSocket) => {
    ...
    const chatSocket = SocketClient(SERVICES.chat, {
        auth      : { userId, email },
...
║  /api/posts         → PostService     :5002      ║
║  /api/users         → UserService     :5003      ║
║  /api/events        → EventService    :5004      ║
║  /api/conversations → MessageService  :5001      ║
║  /api/search        → SearchService   :5005      ║
║  /api/upload        → FileService     :5006      ║
║  ws:/chat           → ChatService     :5007      ║
╚══════════════════════════════════════════════════╝
    `);
});
