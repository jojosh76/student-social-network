...
const ChatService = (() => {
    ...
    return {
        ...
        sendText(text) {
            if (!text.trim()) return;
            const payload = DTOs.ChatMessagePayload({ text });
         ...
         */
        sendImage(imageUrl) {
            const payload = DTOs.ChatMessagePayload({ text: 'Image', imgUrl: imageUrl });
            _socket?.emit('send_message', payload);
        },
         ...
         */
        sendAudio(audioUrl) {
            const payload = DTOs.ChatMessagePayload({ text: 'Note vocale', audioUrl });
            _socket?.emit('send_message', payload);
        },
